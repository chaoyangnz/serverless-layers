...
class ServerlessLayers {
  constructor(serverless, options) {
    ...
    this.hooks = {
      'before:package:function:package': () => BbPromise.bind(this).then(() => {
        return this.init().then(() => this.deployLayers());
      }),
      'before:package:initialize': () => BbPromise.bind(this).then(() => {
        return this.init().then(() => this.deployLayers());
      }),
      'aws:info:displayLayers': () => BbPromise.bind(this).then(() => this.init()).then(() => this.finalizeDeploy()),
      'after:deploy:function:deploy': () => BbPromise.bind(this).then(() => this.init()).then(() => this.finalizeDeploy()),
      'plugin:uninstall:uninstall': () => BbPromise.bind(this).then(() => {
        return this.init().then(() => this.cleanUpAllLayers());
      }),
      'remove:remove': () => BbPromise.bind(this).then(() => {
        return this.init().then(() => this.cleanUpAllLayers());
      })
    };
  }
  ...
  async deployLayers() {
    ...
    for (const layerName in settings) {
      ...
      const enabledFuncs = currentSettings.functions; // deploying a single function

      const deploySingle = !!(cliOpts.function && enabledFuncs); // skip layers that is not related with specified function
  ...
  getSettings() {
    const inboundSettings = (this.serverless.service.custom || {})['serverless-layers'];
  ...
  hasSettingsChanged() {
    ...
    const currentSettings = JSON.stringify(this.settings); // settings checked
    ...
    return this.bucketService.getFile(manifest).then(remoteSettings => {
      ...
      if (!remoteSettings) {
        return this.bucketService.putFile(manifest, currentSettings).then(() => true);
      }
  ...
  async main() {
    ...
    await this.localFolders.init(); // it avoids issues if user changes some configuration
    // which will not be applied till dependencies be changed

    let hasSettingsChanged = await this.hasSettingsChanged(); // check if directories content has changed
    ...
    let hasChanged = !hasFoldersChanged && !hasDepsChanged && !hasSettingsChanged; // merge package default options
  ...
  async getLayerArn() {
    ...
    const arn = (outputs.find(x => x.OutputKey === logicalId) || {}).OutputValue; // cache arn
  ...
  getDependenciesList() {
    return Object.keys(this.localPackage.dependencies || []).map(x => `${x}@${this.localPackage.dependencies[x]}`);
  }
